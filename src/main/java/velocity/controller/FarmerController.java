...
import com.wipro.velocity.efarming.controller.ReturningValue;
...
public class FarmerController {
	...
	@CrossOrigin(origins = "http://localhost:4200")
	@PostMapping("/farmers")
	public ReturningValue loginUser(@Validated @RequestBody Farmer farmer) {
		Boolean a = false;
		...
		ReturningValue returnValue =  new ReturningValue(a, f);
		return returnValue;
	}
	...
	public ResponseEntity<Farmer> updateFarmer(@PathVariable(value = "id") Long id, @Validated @RequestBody Farmer farmer){
		Farmer f = frepo.findById(id).get();
